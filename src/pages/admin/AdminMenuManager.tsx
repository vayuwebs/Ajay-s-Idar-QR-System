...
export default function AdminMenuManager() {
    ...
    const handleAddCategory = async (e: React.FormEvent) => {
        ...
        const { error } = await supabase.from('menu_categories').insert({ name: categoryName, sort_order: newSortOrder });
        if (!error) {
            setCategoryName('');
    ...
    const openEditItemModal = (item: any) => {
        ...
        setImageSource(item.image_url?.startsWith('http') ? 'url' : 'file');
        setImageFile(null);
        setIsItemModalOpen(true);
    };
    ...
    const handleSaveItem = async (e: React.FormEvent) => {
        ...
        if (imageSource === 'url') {
            finalImageUrl = itemImage; // Use the URL directly from the state
        } else if (imageFile) {
            ...
            if (editingItem && editingItem.image_url && !editingItem.image_url.startsWith('http')) {
                // Only try to remove if it was a storage file
                try {
                            ...
                            <div className="mb-4">
                                ...
                                <div className="flex gap-4">
                                    <button
                                        ...
                                        className={`flex-1 py-2 px-4 rounded-lg border text-sm font-medium transition ${imageSource === 'file' ? 'bg-blue-50 border-blue-500 text-blue-700' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'}`}
                                    >
                                        Upload File
                                    </button>
                                    <button
                                        ...
                                        className={`flex-1 py-2 px-4 rounded-lg border text-sm font-medium transition ${imageSource === 'url' ? 'bg-blue-50 border-blue-500 text-blue-700' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'}`}
                                    >
                                        Image URL
                                    </button>
                                </div>
                            ...
                            <div className="mb-4">
                                {imageSource === 'file' ? (
                                    <>
                                        ...
                                        />
                                        {itemImage && !imageFile && !itemImage.startsWith('http') && (
                                    ...
                                    <>
                                        <label className="block text-sm font-medium text-gray-700 mb-1">Google Image URL (Optional)</label>
                                        <input
                                            type="url"
                                            value={itemImage}
